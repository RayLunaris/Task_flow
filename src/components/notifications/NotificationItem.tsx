...
export const NotificationItem: React.FC<NotificationItemProps> = ({ notification, onRead, onDelete }) => {
  ...
  const getIcon = (type: Notification['type']) => {
    switch(type) {
      ...
      case 'team_invite': return <Users size={16} className="text-indigo-500" />;
  ...
  return (
    <div 
      className={clsx(
        "group p-4 flex gap-3.5 border-b border-slate-100 dark:border-slate-800 transition-colors",
    ...
    >
      <div className={clsx("w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-xs", getBgColor(notification.type))}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {isTeamInvite && (
          <div className="mt-2.5 p-3 rounded-xl bg-white dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800 shadow-xs">
