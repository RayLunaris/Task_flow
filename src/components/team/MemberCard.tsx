...
export const MemberCard: React.FC<MemberCardProps> = ({ member, onEdit }) => {
 ...
 whileHover={{ y: -4 }}
 className={clsx(
 "rounded-lg border shadow-sm hover:shadow-md p-5 flex flex-col transition-all duration-300 relative group",
 isInactive || isDeclined
 ? "bg-slate-50/70 dark:bg-[#1A1A1A]/40 border-border-color dark:border-border-color/60 opacity-80"
 : "bg-white dark:bg-[#1A1A1A] border-border-color dark:border-border-color"
 )}
 >
 {/* Top Section */}
 ...
 member.role === 'manager' ? "bg-subtle text-primary dark:bg-blue-900/40 dark:text-primary" :
 member.role === 'client' ? "bg-purple-100 text-primary dark:bg-purple-900/40 dark:text-purple-300" :
 ...
 {isInactive && (
 <span className="text-[10px] px-2 py-0.5 rounded-full font-bold uppercase bg-slate-200 text-slate-600 dark:bg-[#242424] dark:text-slate-400">
 Nonaktif
 </span>
 )}
 </div>
 ...
 {showMenu && (
 <div className="absolute right-0 top-full mt-1 w-44 bg-white dark:bg-[#242424] rounded-lg shadow-sm border border-slate-100 dark:border-border-color py-1.5 z-20 animate-in fade-in slide-in-">
 {isInvited && (
 <>
 <button
 ...
 className="w-full text-left px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2 cursor-pointer"
 >
 <Send size={14} className="text-primary" /> Kirim Ulang Email
 </button>
 ...
 {/* Workload Mini Grid */}
 <div className="mt-auto pt-3.5 border-t border-slate-100 dark:border-border-color/80">
 <div className="grid grid-cols-3 gap-2 text-center">
 <div className="bg-subtle dark:bg-blue-900/20 rounded-lg p-2">
 <div className="text-base font-black text-primary dark:text-primary">{activeTasks}</div>
 ...
 <div className={`rounded-lg p-2 ${overdueTasks > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-slate-50 dark:bg-[#242424]/40'}`}>
