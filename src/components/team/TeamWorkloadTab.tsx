...
export const TeamWorkloadTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-500/10 via-teal-500/10 to-transparent dark:from-blue-900/30 dark:via-teal-900/20 p-5 rounded-2xl border border-blue-200/60 dark:border-blue-800/40 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {availableMembers.map(({ member, activeCount }) => (
            <div
              key={member.id}
              className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs"
            >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl py-2 pl-9 pr-4 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 text-slate-800 dark:text-slate-100"
          />
        </div>
      ...
      <div className="space-y-3">
        {filteredMembers.map(({ member, activeCount, doneCount, reviewCount, overdueCount, onTimeRate }) => {
          ...
          return (
            <div
              key={member.id}
              className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-5 hover:border-primary/40 transition-all"
            >
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={clsx(
                        ...
                        member.role === 'manager' ? 'bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300' :
                        member.role === 'client' ? 'bg-purple-100 text-purple-700 dark:bg-purple-950 dark:text-purple-300' :
                ...
                <div className="grid grid-cols-4 gap-2 text-center text-xs shrink-0">
                  ...
                  <div className="bg-blue-50 dark:bg-blue-950/30 p-2 rounded-xl border border-blue-100 dark:border-blue-900/40">
                    <span className="block font-black text-blue-600 dark:text-blue-400 text-sm">{reviewCount}</span>
                    <span className="text-[10px] text-blue-600 dark:text-blue-400 uppercase font-semibold">Review</span>
                  </div>
                ...
                <div className="flex lg:flex-col items-center lg:items-end justify-between border-t lg:border-t-0 pt-3 lg:pt-0 border-slate-100 dark:border-slate-800 min-w-[110px]">
